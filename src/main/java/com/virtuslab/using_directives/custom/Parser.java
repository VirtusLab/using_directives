...
public class Parser {
    ...
    UsingDefs usingDirectives() {
        ...
        UsingDef ud = usingDirective(isFirstTokenInLine);
    ...
    UsingDef usingDirective(Boolean isFirstTokenInLine) {
        ...
        if (tokenData.token != Tokens.EOF && !isFirstTokenInLine) {
            error(String.format("Expected white space after the using directivce, in the line; but found %s", tokenData.toTokenInfoString()));
        }
        return null;
    }
