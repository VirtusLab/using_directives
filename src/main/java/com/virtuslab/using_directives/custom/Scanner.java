            ...
            case COMMA:
                if(currentRegion instanceof Indented && isEnclosedInParens(currentRegion.outer())) {
                    ...
                    if(td.isAfterLineEnd()
                        && (td.token == Tokens.RPAREN || td.token == Tokens.RBRACKET || td.token == Tokens.RBRACE || td.token == Tokens.OUTDENT)
                    ) { }
...
//                observeColonEOL();
    ...
    private void fetchToken() {
        ...
        if(ch == ' ' || ch == '\t' || ch == CR || ch == LF || ch == FF) {
            ...
            if(isIdentifierStart(reader.ch)) {
                charLitOr(() -> { getIdentRest(); return Tokens.QUOTEID; });
            } else if(isIdentifierPart(reader.ch) && reader.ch != '\\') {
                charLitOr(() -> { getOperatorRest(); return Tokens.QUOTEID; });
            } else {
                switch(reader.ch) {
                    case '{':
                    case '[':
    ...
    public boolean isSoftModifier() {
        return td.token == Tokens.IDENTIFIER
