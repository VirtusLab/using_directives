...
public class CustomUsingValueAdapter implements JsonDeserializer<UsingValue>, JsonSerializer<UsingValue> {
    ...
    @Override
    public UsingValue deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
        JsonObject jsonObj = json.getAsJsonObject();
        String type = jsonObj.get("type").getAsString();
        Position pos = context.deserialize(jsonObj.getAsJsonObject("position"), Position.class);
        if(type.equals("list")) {
            return new UsingValues(
                    new ArrayList<>(Arrays.asList(context.deserialize(jsonObj.get("value").getAsJsonArray(), UsingPrimitive[].class))),
                    pos
            );
        } else {
    ...
    public JsonElement serialize(UsingValue src, Type typeOfSrc, JsonSerializationContext context) {
        ...
        if(src instanceof UsingValues) {
            ...
            JsonElement serializedValues = context
                    .serialize(((UsingValues) src).values.toArray(), UsingPrimitive[].class);
