...
public class Test extends TestUtils {
    ...
    public void test() throws IOException, URISyntaxException {
        ...
        UsingTree ast = new Parser(new Source(getContent("parser_tests/inputs/testcase14.txt").toCharArray()), ctx).parse();
        System.out.println(ast);
        System.out.println(gson.toJson(ast));
        int codeOffset = ((UsingDefs) ast).getCodeOffset();
        System.out.println(getContent("comment_extractor_tests/inputs/comment1.txt").substring(codeOffset));
        System.out.println(new Visitor(ast, ctx).visit().getFlattenedMap());
        UsingDirectives ud = new UsingDirectivesProcessor().extract(getContent("comment_extractor_tests/inputs/comment2.txt").toCharArray());
        System.out.println(ud.getFlattenedMap());
        codeOffset = ud.getCodeOffset();
        System.out.println(ud.getCodeOffset());
        System.out.println(getContent("comment_extractor_tests/inputs/comment2.txt").substring(codeOffset));
    }
